...
# --- 14 Official School Activities (UP Board Co-Curricular & Academic Calendar) ---
DEFAULT_ACTIVITIES = [
...
st.caption("Google Form All-in-One Response Sheet Upload एवं 1-क्लिक UP Board पोर्टफोलियो जनरेटर")
...
with tab_upload:
    st.subheader("📥 Google Form All-in-One रिस्पॉन्स फ़ाइल अपलोड करें (.xlsx / .csv)")
    ...
    if uploaded_file is not None:
        try:
            ...
            st.write("Uploaded Data Preview (First 2 rows):", df_resp.head(2))
            
            if st.button("🚀 Upload & Sync to Portfolios (डेटा सुरक्षित सेव करें)", type="primary"):
                c = conn.cursor()
                success_count = 0
                
                # Activity quick lookup maps
                act_date_map = {act["name"]: act["date"] for act in DEFAULT_ACTIVITIES}
                act_cat_map = {act["name"]: act["cat"] for act in DEFAULT_ACTIVITIES}
                
                for _, r in df_resp.iterrows():
                    # 1. Detect Student Roll Number
                    r_no = ""
                    ...
                    # 2. Iterate through all 14 Activities to capture submitted data
                    for act in DEFAULT_ACTIVITIES:
                        act_full_name = act["name"]
                        # Clean match token (e.g. 'रंगोली प्रतियोगिता' from '2. रंगोली प्रतियोगिता')
                        act_token = act_full_name.split(".", 1)[-1].strip() if "." in act_full_name else act_full_name
                        ...
                        for col in df_resp.columns:
                            col_str = str(col)
                            if act_full_name in col_str or act_token in col_str:
                                col_low = col_str.lower()
                                if any(k in col_low for k in ["desc", "विवरण", "कार्य"]):
                                    desc_val = clean_val(r[col])
                                elif any(k in col_low for k in ["refl", "सीख", "चिंतन", "learn"]):
                                    refl_val = clean_val(r[col])
                                elif any(k in col_low for k in ["photo", "फोटो", "चित्र", "link", "drive", "upload"]):
                                    link_val = clean_val(r[col])
                        
                        # Agar student ne is activity me data bhara hai
                        if desc_val or refl_val or link_val:
                            direct_img = convert_gdrive_link(link_val)
                            act_date = act_date_map.get(act_full_name, datetime.now().strftime("%d.%m.%Y"))
                            act_cat = act_cat_map.get(act_full_name, "सह-पाठ्यचर्या")
                            today_str = datetime.now().strftime("%d-%m-%Y")

                            # Duplicate check taaki entries repeat na hon
                            c.execute("""
                                SELECT id FROM portfolio_entries 
                                WHERE roll_no=? AND activity_name=?
                            """, (r_no, act_full_name))
                            existing = c.fetchone()
                            ...
                            if existing:
                                c.execute("""
                                    ...
                                    SET student_description=?, student_reflection=?, evidence_link=?
                                    WHERE id=?
                                """, (desc_val, refl_val, direct_img, existing[0]))
                            else:
                                c.execute("""
                                ...
                                """, (r_no, act_full_name, act_cat, act_date, desc_val, refl_val, direct_img, today_str))

                            # Agar photo link hai to student master me attach karein
                            if direct_img:
                ...
                conn.commit()
                if success_count > 0:
                    st.success(f"🎉 {success_count} गतिविधियाँ सफलतापूर्वक छात्र पोर्टफोलियो में सुरक्षित रूप से दर्ज हो गईं!")
                    st.rerun()
                else:
                    st.warning("फ़ाइल में कोई नई गतिविधि प्रविष्टि नहीं मिली।")
