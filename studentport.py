...
# Function to automatically read studentport.xlsx from GitHub/Domain folder
def sync_excel_data():
    ...
    if not target_file:
        return 0, "studentport.xlsx file directory me nahi mili."

    try:
        df_excel = pd.read_excel(target_file)
        conn = get_db_connection()
        c = conn.cursor()
        success_count = 0
        ...
        for _, row in df_excel.iterrows():
            ...
            if not r_no or r_no == "nan" or r_no == "":
                continue
...
if not st.session_state.logged_in:
    ...
    with col2:
        st.info("""
        **Default Logins:**
        - **Teacher (Admin):** - Username: `admin` | Password: `admin123`
        - **Students:** - Username: *Apna Roll Number (e.g. jo sheet me hai)*
...
# --- Authenticated App ---
    ...
    if role == "Teacher":
        ...
        # --- TAB 3: Sync with studentport.xlsx ---
        with tabs[2]:
            st.subheader("GitHub / Domain File Sync (`studentport.xlsx`)")
            st.write("Agar aapne GitHub repo ya domain par `studentport.xlsx` file update ki hai, toh yahan click karke database sync karein:")
            
            if st.button("🔄 Sync / Re-load Data from studentport.xlsx", type="primary"):
                count, msg = sync_excel_data()
                if count > 0:
                    st.success(f"{count} students ka data successfully database me sync ho gaya!")
                    st.rerun()
                else:
                    st.error(f"Sync failed: {msg}")

        # --- TAB 4: Add Single Student Manually ---
        with tabs[3]:
            st.subheader("Add Student Information Manually")
            with st.form("single_student_form"):
                ...
                if save_student:
                    if not r_no or not s_name:
                        st.error("ROLL NO aur STUDENT'S NAME zaroori hain!")
                    else:
                        dob_str = dob.strftime("%d/%m/%Y")
                        dd, mm, yyyy = dob.strftime("%d"), dob.strftime("%m"), dob.strftime("%Y")
                        try:
                            c = conn.cursor()
                            c.execute("""
                            ...
                            """, (
                                r_no, r_no, p_word, sr_no, roll_10th, 
                            ...
                            conn.commit()
                            st.success(f"Student {s_name} add ho gaya!")
                        except sqlite3.IntegrityError:
                            st.error("Ye Roll Number pehle se database me maujood hai!")
